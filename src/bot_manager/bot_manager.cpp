...
TgBot::InlineKeyboardMarkup::Ptr BotManager::mModifyMaterialMenu(new TgBot::InlineKeyboardMarkup);
...
void BotManager::initMenus() {
    ...
    materials_menu_modify_material->text         = "Оновити матеріал";
    ...
    mMaterialsMenu->inlineKeyboard.push_back(
        {materials_menu_add_material, materials_menu_modify_material});
...
void BotManager::callbackOnAnyMessage(const TgBot::Message::Ptr& message) {
    insertUserInTableItNotExists(message);
    ...
    if (client_status->do_user_type_material_name) {
        ...
        if (find_result) {
            const auto send_str = fmt::format("Наразі {} {}{} вже в базі даних", find_result->name,
                                              find_result->count, find_result->suffix.value_or(""));
        ...
        client_status->typed_material_row.name         = message->text;
        ...
        if (isFirstTime) {
            isFirstTime = false;
            TgBot::KeyboardButton::Ptr material_suffix_no_suffix_button(new TgBot::KeyboardButton);
            ...
            material_suffix_reply_keyboard->keyboard.push_back({material_suffix_no_suffix_button});
        }

        mBotHandler.getApi().sendMessage(message->chat->id, "В чому вимірюється одиниця матеріалу?",
                                         false, 0, material_suffix_reply_keyboard);
        ...
        if (message->text != "Матеріал не має одиниці виміру") {
            client_status->typed_material_row.suffix = message->text;
        }
        ...
        try {
            client_status->typed_material_row.count = std::stod(message->text);
        ...
        if (mDatabaseManager.addMaterial(client_status->typed_material_row)) {
            const auto send_str = fmt::format(
                "{} {}{} внесено в базу даних", client_status->typed_material_row.name,
                client_status->typed_material_row.count,
                client_status->typed_material_row.suffix.value_or(""));
            mBotHandler.getApi().sendMessage(message->chat->id, send_str);
        } else {
            mBotHandler.getApi().sendMessage(
                message->chat->id, "На жаль сталась помилка. Зверніться до адміністратора");
        ...
        if (mDatabaseManager.updateMaterialCountById(client_status->updating_material.count,
                                                     client_status->updating_material.id.value())) {

            const auto send_str = fmt::format("{} {}{} оновлено в базі даних",
...
void BotManager::callbackOnCallbackQuery(const TgBot::CallbackQuery::Ptr& query) {
    ...
    if (query->data == "main_menu") {
        ...
        mModifyMaterialMenu->inlineKeyboard.clear();
        ...
        for (; i < materials.size() - materials.size() % max_columns_size; i += max_columns_size) {
            ...
            mModifyMaterialMenu->inlineKeyboard.push_back(row);
        }
        if (i < materials.size()) {
            ...
            mModifyMaterialMenu->inlineKeyboard.push_back(last_row);
        }
        ...
        mModifyMaterialMenu->inlineKeyboard.push_back({back_button});
        ...
        client_status->updating_material
            = mDatabaseManager.getMaterialById(client_status->updating_material.id.value()).value();
        mBotHandler.getApi().sendMessage(
            query->message->chat->id,
            fmt::format("Введіть нову кількість для {} {}{}", client_status->updating_material.name,
                        client_status->updating_material.count,
                        client_status->updating_material.suffix.value_or("")));
    }
}
...
std::shared_ptr<ClientChatStatus> BotManager::getClientChatStatus(int64_t client_id) {
    ...
    auto it = mClientChatStatuses.find(client_id);
...
void BotManager::insertUserInTableItNotExists(const TgBot::Message::Ptr& message) {
    auto found_user = mDatabaseManager.getUserByTelegramId(message->from->id);
    if (!found_user) {
        ...
        mDatabaseManager.addCustomer(user_row);
    }
}
